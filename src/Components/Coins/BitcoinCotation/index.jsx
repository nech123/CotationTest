...
import{Container, IconBitcoin} from './styles.js'

export function BitcoinCotation(){

  const [BTC, setBTC] = useState('null')
  const [BTCmax, setBTCmax] = useState('null')
  const [BTCmin, setBTCmin] = useState('null')
  ...
  const getData = async() =>{
    ...
    setBTC(res.data.BTCBRL.ask)
    setBTCmax(res.data.BTCBRL.high)
    setLastAtt(res.data.BTCBRL.create_date)
    setNameMoeda(res.data.BTCBRL.name)
    setBTCmin(res.data.BTCBRL.low)
  }
  ...
  const getDays = async() =>{
    const days = await axios.get('https://economia.awesomeapi.com.br/json/daily/BTC-BRL/16', 
    { params:{USDBRL: [1]}})
  ...
  return(
    <Container>
        ...
        <IconBitcoin/>
        <span className='attBTC'>(Ultima atualização em: {lastAtt})</span>
      <div className='div-BTC'>
      
      <span>{nameMoeda} </span>
      <span className='BTC'>{new Intl.NumberFormat('pt-BR', {
                  style:'currency',
                  currency: 'BRL'
                }).format(BTC)}
      ...
      <div className="maxBTC">

      <BsArrowUpRight className='IoMdArrowDropupCircle'/><span className='max-BTC'> Valor maximo hoje: {new Intl.NumberFormat('pt-BR', {
                  style:'currency',
                  currency: 'BRL'
                }).format(BTCmax)} </span>
                </div>
